...
import com.kunjava.domain.QueryVo;
...
public class MybatisTest {
    ...
    public void testSave(){
        User user = new User();
        user.setUserName("当家的");
        user.setUserAddress("海口市");
        user.setUserSex("女");
    ...
    public void testFindByName(){
        //5.执行查询一个方法
        List<User> users = userMap.findByName("老%");
//        List<User> users = userMap.findByName("王");
        for(User user : users){
            System.out.println(user);
        }
    }
    ...
    public void testFindByVo(){
        ...
        vo.setUser(user);
        //5.执行查询一个方法
        List<User> users = userMap.findUserByVo(vo);
        for(User u : users){
